...
// Create a default theme so we can reuse pxToRem and default shadows
const defaultTheme = createTheme();

// Start with a copy so we can overrimport { createTheme, alpha } from '@mui/material/styles';
...
// Build design tokens for v5 createTheme({ palette }) usage
export const getDesignTokens = (mode) => {
  // Override one shadow index depending on mode
  customShadows[1] =
    mode === 'dark'
  ...
  return {
    palette: {
      ...
      // Custom key is fine in JS; TS augmentation is not needed here
      baseShadow:
        mode === 'dark'
          ? 'hsla(220, 30%, 5%, 0.7) 0px 4px 16px 0px, hsla(220, 25%, 10%, 0.8) 0px 8px 16px -5px'
          : 'hsla(220, 30%, 5%, 0.07) 0px 4px 16px 0px, hsla(220, 25%, 10%, 0.07) 0px 8px 16px -5px',
...
// MUI v6 colorSchemes export (used by your AppTheme.jsx)
...
// Make shadow index 1 pull from the palette CSS variable you set via colorSchemes
// In TS you used a Shadows type; in JS we just build the array directly.
