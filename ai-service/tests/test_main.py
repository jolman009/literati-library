...
class TestSummarizeEndpoint:
    ...
    def test_summarize_with_max_length(
        self, test_client, mock_gemini_client, sample_note_text
    ):
        """Test summarization with custom max length."""
        response = test_client.post(
            "/summarize-note", json={"text": sample_note_text, "max_length": 50}
        )
    ...
    def test_summarize_empty_text(self, test_client, empty_note_text):
        ...
        response = test_client.post("/summarize-note", json={"text": empty_note_text})

        assert response.status_code == 422  # Validation error

    def test_summarize_missing_text(self, test_client):
        """Test summarization without text field."""
        response = test_client.post("/summarize-note", json={})
    ...
    def test_summarize_invalid_max_length(self, test_client, sample_note_text):
        ...
        response = test_client.post(
            "/summarize-note", json={"text": sample_note_text, "max_length": -1}
        )

        assert response.status_code == 422  # Validation error

    def test_summarize_api_error(
        self, test_client, mock_gemini_error, sample_note_text
    ):
    ...
    def test_summarize_special_characters(
        self, test_client, mock_gemini_client, special_characters_note
    ):
        """Test summarization with special characters."""
        response = test_client.post(
            "/summarize-note", json={"text": special_characters_note}
        )
    ...
    def test_summarize_multilingual(
        self, test_client, mock_gemini_client, multilingual_note
    ):
        """Test summarization with multilingual text."""
        response = test_client.post("/summarize-note", json={"text": multilingual_note})
    ...
    def test_summarize_long_text(self, test_client, mock_gemini_client, long_note_text):
        ...
        response = test_client.post("/summarize-note", json={"text": long_note_text})
    ...
    def test_summarize_performance(
        self, test_client, mock_gemini_client, performance_test_data
    ):
        ...
        for text_type, text in performance_test_data.items():
            ...
            response = test_client.post("/summarize-note", json={"text": text})
    ...
    async def test_summarize_async(
        self, async_client, mock_gemini_client, sample_note_text
    ):
        """Test summarization with async client."""
        response = await async_client.post(
            "/summarize-note", json={"text": sample_note_text}
        )
    ...
    def test_summarize_security_malicious_input(
        self, test_client, mock_gemini_client, malicious_note_text
    ):
        """Test handling of potentially malicious input."""
        response = test_client.post(
            "/summarize-note", json={"text": malicious_note_text}
        )
...
class TestErrorHandling:
    ...
    def test_api_quota_exceeded(
        self, test_client, mock_api_quota_exceeded, sample_note_text
    ):
...
class TestRateLimiting:
    ...
    def test_rate_limiting(self, test_client, mock_gemini_client, sample_note_text):
        ...
        for i in range(20):
            response = test_client.post(
                "/summarize-note", json={"text": f"{sample_note_text} {i}"}
            )
...
class TestInputValidation:
    ...
    def test_max_text_length(self, test_client):
        ...
        response = test_client.post("/summarize-note", json={"text": very_long_text})
    ...
    def test_text_type_validation(self, test_client):
        ...
        response = test_client.post(
            "/summarize-note", json={"text": 12345}  # Number instead of string
        )
    ...
    def test_max_length_bounds(self, test_client, sample_note_text):
        ...
        response = test_client.post(
            "/summarize-note", json={"text": sample_note_text, "max_length": 100000}
        )
        ...
        response = test_client.post(
            "/summarize-note", json={"text": sample_note_text, "max_length": 0}
        )
...
class TestIntegration:
    ...
    def test_real_api_integration(self, test_client):
        ...
        assert (
            data["summary"]
            != "This is a real test of the AI summarization service. It should create a meaningful summary of this text."
        )
